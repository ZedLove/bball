...
describe('EVENT_TYPE_CATEGORY_MAP', () => {
  describe('plate-appearance-completed categories', () => {
    it.each([
      ...
      expect(EVENT_TYPE_CATEGORY_MAP.get(eventType)).toBe(
  ...
  describe('substitution categories', () => {
    it('maps "pitching_substitution" to "pitching-substitution"', () => {
      expect(EVENT_TYPE_CATEGORY_MAP.get('pitching_substitution')).toBe(
        'pitching-substitution'
      );
    });

    it('maps "offensive_substitution" to "offensive-substitution"', () => {
      expect(EVENT_TYPE_CATEGORY_MAP.get('offensive_substitution')).toBe(
        'offensive-substitution'
      );
    });

    it('maps "defensive_substitution" to "defensive-substitution"', () => {
      expect(EVENT_TYPE_CATEGORY_MAP.get('defensive_substitution')).toBe(
        'defensive-substitution'
      );
    });

    it('maps "defensive_switch" to "defensive-substitution"', () => {
      expect(EVENT_TYPE_CATEGORY_MAP.get('defensive_switch')).toBe(
        'defensive-substitution'
      );
    });
  });
  ...
  describe('suppressed action types', () => {
    it.each([
      ...
      'mound_visit',
      'defensive_indiff',
    ])('returns undefined for suppressed type "%s"', (eventType) => {
  ...
  describe('unknown event types', () => {
    it('returns undefined for completely unknown event types', () => {
      expect(
        EVENT_TYPE_CATEGORY_MAP.get('totally_unknown_event')
      ).toBeUndefined();
    });
